...
fileprivate struct Ordering {
    var pagesBefore: [Int]
    var pagesAfter: [Int]
    
    init(pagesBefore: [Int] = [], pagesAfter: [Int] = []) {
        self.pagesBefore = pagesBefore
        self.pagesAfter = pagesAfter
    }
}

fileprivate extension [Int: Ordering] {
    mutating func set(lowerPage: Int, higherPage: Int) {
        for page in [lowerPage, higherPage] {
            if !self.keys.contains(page) {
                self[page] = .init()
            }
        }

        self[lowerPage]?.pagesAfter.append(higherPage)
        self[higherPage]?.pagesBefore.append(lowerPage)
    }
}

class Day5Part2: Day {
    private var orderMap: [Int: Ordering] = [:]
    ...
    func run(input: String) throws -> String {
        ...
        let pagePairs = parts.first!
            .components(separatedBy: .newlines)
            .map { line in
                let pageNumbers = line.components(separatedBy: "|").map { Int($0)! }
                return (pageNumbers[0], pageNumbers[1])
            }
        
        // Build the order map
        for (lowerPage, higherPage) in pagePairs {
            orderMap.set(lowerPage: lowerPage, higherPage: higherPage)
        }
        
        // Check the valid updates
        let updates = parts[1]
            .components(separatedBy: .newlines)
            .map { line in
        ...
        for update in updates {
            print("Checking update \(update.map(String.init).joined(separator: ", "))")
            if !isUpdateValid(update) {
                // Fix the update
                let fixedUpdate = fix(update)
                
                // Add the middle number to the sum
                assert(fixedUpdate.count % 2 == 1)
    ...
    private func fix(_ update: [Int]) -> [Int] {
        update.sorted { page1, page2 in
            guard page1 != page2 else { return false }
            guard let order = orderMap[page1] else { return false }

            return order.pagesAfter.contains(page2)
        }
    }
    
    private func isUpdateValid(_ update: [Int]) -> Bool {
        for (index, page) in update.enumerated() {
            // If we have no order for a page, we continue normally
            if let order = orderMap[page] {
                // Check if following numbers are not in pagesBefore
                for laterPage in update[index...].dropFirst() {
                    guard !order.pagesBefore.contains(laterPage) else {
                        print("  Page \(page) must be ordered after \(laterPage).")
                        return false
                    }
                }
                for earlierPage in update[...index].dropLast() {
                    guard !order.pagesAfter.contains(earlierPage) else {
                        print("  Page \(page) must be ordered before \(earlierPage).")
                        return false
                    }
                }
            }
    ...
    var expectedTestResult: String = "123"
